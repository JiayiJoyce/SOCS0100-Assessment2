...
The following contains the setup code.
...
I then collect music-related data through Spotify API as it offers an interface to relevant databases, which include up to date popular playlists and data on relevant audio features. Furthermore, the [spotifyr](https://www.rcharlie.com/spotifyr/) package provided a wrapper which made it easier for me to integrate the API output.
...
In the end, I produced a tidied data frame: `wrangled_IFPI_data`.
...
The end product is a tidied dataframe: `wrangled_tophits`.

##### Overview of datasets

The `wrangled_IFPI_data` dataframe contains the value of music markets in different countries from 2011 to 2017, with 8 columns and 77 rows. It showcases the percentage of different facets of the music market (`physical`, `digital`, `performance rights`) in countries with leading market sizes and therefore have implications in the study of the concentration of the media [@geurts2023]in social science.
...
The `wrangled_tophits` dataframe contains the audio features (including `valence`, `dancability`, etc.) of the top 50 songs in 6 countries, with 14 columns and 300 rows. The six countries are selected as the size of their music market are by far the largest according to the IFPI data. Most of these audio features are numeric and rated from 0-1, making it easy for cross comparison. Others, such as `track.explicit` and `track.name` are logical or characters. It provide an up-to-date insight on the musical taste, an important cultural aspect[@aarons2018], in countries with the most developed music industry.

The following analysis will be focusing on five dimensions: Dancibility, Valence, Energy, Speachiness and Instrumentalness. They are metrics developed by Spotify that measures audio features on a scale of 0-1. Detailed definition of variables on audio features by the official Spotify API site can be found [here](https://developer.spotify.com/documentation/web-api/reference/get-audio-features).
...
The following diagram is suitable for showcasing whether specific audio features correlates with `popularity` under different cultural contexts. Each observation is one of the `Top 50 hit songs`. Upon hovering, specific name of the song that makes up the datapoints is shown.
...
-   Further analysis about correlation can be built upon this,although upon glance tophits in Japan are highly concentrated in terms of popularity- most fall between the 70-80 range, while similar phenomenon isn't observed elsewhere.
...
The following radar diagram provides a direct holistic comparison between the avearage of essential audio features of top 50 songs in the six countries. Radargraph is choosen because the five variables are all rated on the same scale (from 0-1) by spotify.

-   Popular songs in Japan is weakest in terms of `energy` and `valence`. This provide grounds for future research regarding the Cultual difference.

-   Unlike plotly, `fmsb` pacakge that generates radarchart does not contain download feature, so I wrote a link for download below the graph.
...
downloadLink("downloadRadar", "Download Radar Chart for your choosen country")

```

This diagram shows changes in major music markets and their level of digitization.

-   It showcased that while US and Japan has two of the largest music markets, they operate in drastically different trends.

-   During the process of digitization, US's market continued to grow in size drastically, as showcased by the size of the pink dot.

-   While US is rapidly digitizing during 2011-2017, Japan's market remained highly physical.

-   This indicates that top hits collected from Spotify, an on-line platform, may not be representative of the average music taste in Japan.
...
The code for an animated version that can be manually activated is attached below, where shadows are added to better showcase the dynamics.

```{r}
#| label: animated market chart
#| echo: true
#| eval: false
...
# 2. run the setup chunk at the top, then scroll to the bottom and run the server code chunk, ignore the error message- this step is done to import the wrangled data into the environment
...
Although there are many times when ChatGPT failed to provide me with a runnable solution (roughly 8 out of 10 times the code won't run), it was suitable for making simple refinements. Compared to solutions on stackoverflow or CRAN, its is tailored to my code and therefore saves the time compared to adapting internet solution to my own code.

There are other ways in which ChatGPT supported my computational process. I used the explanatory function of ChatGPT as a form of dictionary throughout the process, as it can breaks code chunks down into steps and explain each line. This function allowed me to continuously reinforce my memory of the coding language throughout the process of coding.

## Part III Server code (view in r studio)
